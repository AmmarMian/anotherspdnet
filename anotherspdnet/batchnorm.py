...
def riemannian_mean_spd(
    ...
    with torch.no_grad():
        ...
        for _ in range(max_iter):
            ...
            C12 = eigvecs @ torch.diag_embed(sqrt_eigvals) @ eigvecs.transpose(-2, -1)
            C12_inv = eigvecs @ torch.diag(inv_sqrt_eigvals) @ eigvecs.transpose(-2, -1)

            # Getting the tangent space matrices
            eigvals, eigvecs = torch.linalg.eigh(C12_inv @ X @ C12_inv)
            log_eigvals = torch.log(eigvals)
            log_matrices = torch.einsum(
                "...ij,...jk,...kl->...il",
                eigvecs,
            ...
            exp_J = eigvecs @ torch.diag(exp_eigvals) @ eigvecs.transpose(-2, -1)
            mean = C12 @ exp_J @ C12
...
class BatchNormSPD(nn.Module):
    ...
    def forward(self, X: torch.Tensor):
        ...
        if self.training:
            ...
            new_mean = riemannian_mean_spd(
                X,
                initial_stepsize=self.initial_stepsize_mean,
        ...
        X_centered = torch.einsum(
            "ij,...jk,kl->...il", isqrtm_new_mean, X, isqrtm_new_mean
        )
        X_normalized = torch.einsum(
            "ij,...jk,kl->...il", sqrtm_bias, X_centered, sqrtm_bias
        )
    ...
    def __repr__(self):
        return (
            f"BatchNormSPD({self.n_features}, momentum={self.momentum}, "
